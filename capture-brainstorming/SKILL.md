...
- Output format: themed Markdown bullets grouped by topic (NOT by session), plus a short list of which session files actually contributed. Follow the global no-AI-signal writing rule in the user's `CLAUDE.md` (no em dashes, no "crucial/delve/pivotal/showcase", straight quotes, no rule-of-three padding). No Unicode box-drawing characters in any table.
