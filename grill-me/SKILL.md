...
model: fable
name: grill-me
description: Interview the user relentlessly about a plan, design, or document, walking each branch of the decision tree and resolving dependencies one at a time, until every weak spot is surfaced and either fixed or explicitly accepted. Asks one question at a time, gives a recommended answer for each, and explores the codebase to answer questions it can rather than asking. Accepts an optional file path (e.g., "/grill-me BRAINSTORM.md", "/grill-me SPEC.md"); otherwise grills whatever plan is in the conversation. Reusable across the chain, so you can stress-test a rough idea before picking a path, then re-run it on REQUIREMENTS.md and SPEC.md. Use when the user wants their plan challenged or stress-tested, wants to get grilled on their design, says "grill me", "stress-test this", "poke holes in", or wants a reviewer's pass before committing to a direction. In a repo with a CONTEXT.md glossary or docs/adr/ (or on request), also maintains the docs trail and offers ADRs for one-way decisions. Adversarial but constructive.
